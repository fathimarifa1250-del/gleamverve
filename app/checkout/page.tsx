...
export default function CheckoutPage() {
  ...
  const applyCoupon = () => {
    ...
    if (code === "WELCOME10") setDiscount(subtotal * 0.1);
    else if (code === "SAVE200") setDiscount(200);
    else if (code === "FREESHIP") setDiscount(shipping);
    else {
      setDiscount(0);
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-[2rem] p-8 border border-[#eadfce]">
          ...
          <div className="grid gap-5">
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Full Name" className="border p-4 rounded-xl" />
            <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number" className="border p-4 rounded-xl" />
            <textarea value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" className="border p-4 rounded-xl h-28" />
            <input value={city} onChange={(e) => setCity(e.target.value)} placeholder="City" className="border p-4 rounded-xl" />
            <input value={pin} onChange={(e) => setPin(e.target.value)} placeholder="PIN Code" className="border p-4 rounded-xl" />

            <select value={state} onChange={(e) => setState(e.target.value)} className="border p-4 rounded-xl">
          ...
          <div className="flex gap-2 mt-6">
            <input value={coupon} onChange={(e) => setCoupon(e.target.value)} placeholder="Coupon Code" className="flex-1 border p-3 rounded-xl" />
            <button onClick={applyCoupon} className="bg-[#5a422a] text-white px-6 rounded-xl">
              Apply
            </button>
          </div>
